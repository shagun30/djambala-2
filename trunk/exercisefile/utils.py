...
def get_folder_name(item_container, is_protected):
  ...
  if is_protected:
    return DOWNLOAD_PROTECTED_PATH + item_container.container.path
  else:
    return DOWNLOAD_PATH + item_container.container.path

# -----------------------------------------------------
def get_file_name(item_container, is_protected):
  """ liefert den Pfad der Datei """
  if is_protected:
    return DOWNLOAD_PROTECTED_PATH + item_container.container.path + item_container.item.name
  else:
    return DOWNLOAD_PATH + item_container.container.path + item_container.item.name
...
def get_file_url(item_container, is_protected=False):
  ...
  filename = get_file_name(item_container, is_protected)
  if is_protected:
    return get_site_url(item_container, item_container.item.name) + '/download/'
  else:
    return DOWNLOAD_URL + item_container.container.path + item_container.item.name
...
def get_file_path(item_container):
  ...
  if item_container.item.app.is_folderish:
    return file_path + item_container.container.path
  else:
    return file_path + item_container.container.path + item_container.item.name

# -----------------------------------------------------
def save_file(name, files, item_container):
  """ Datei speichern """
