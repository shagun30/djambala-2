# -*- coding: utf-8 -*-
"""
/dms/eduwebquestitem/utils.py

.. enthaelt Hilfefunktionen fuer Webquests in Lernarchiven
...
def get_user_support(item_container, authenticated):
  ...
  if item_container.item.has_user_support and authenticated:
    tSection = get_template('app/eduwebquestitem/user_support.html')
    cSection = Context ({ 'path': get_site_url(item_container, ''), })
...
def check_webquest(item_container, vars):
  ...
  if parent.item.app.name == 'dmsEduWebquestItem':
