...
from dms.auth import authenticate
...
class UserCreationForm(oldforms.Manipulator):
    ...
    def __init__(self):
        self.fields = (
            ...
            oldforms.PasswordField(field_name='password2', length=30, max_length=60, is_required=True,
                validator_list=[validators.AlwaysMatchesOtherField('password1', _(u"The two password fields didn't match."))]),
        )
    ...
    def isValidUsername(self, field_data, all_data):
        try:
            ...
            return
        raise validators.ValidationError, _(u'A user with that username already exists.')
...
class AuthenticationForm(oldforms.Manipulator):
    ...
    def hasCookiesEnabled(self, field_data, all_data):
        if self.request and not self.request.session.test_cookie_worked():
            raise validators.ValidationError, _(u"Your Web browser doesn't appear to have cookies enabled. Cookies are required for logging in.")

    def isValidUser(self, field_data, all_data):
        username = field_data
        password = all_data.get('password', None)
        org_id = all_data.get('org_id', 0)
        # Test auf username
        # Test auf orgid_username
        # Test auf sch_orgid_username
        self.user_cache = authenticate(username=username, password=password)
        if self.user_cache is None:
            raise validators.ValidationError, _(u"Please enter a correct username and password. Note that both fields are case-sensitive.")
        elif not self.user_cache.is_active:
            raise validators.ValidationError, _(u"This account is inactive.")
        from dms.settings import HAS_HOMES
        if HAS_HOMES:
          from dms.queries import has_quota, create_quota, has_home,create_home
          if not has_quota(username):
            create_quota(username)
          if not has_home(self.user_cache):
            create_home(self.user_cache)
...
class PasswordResetForm(oldforms.Manipulator):
    ...
    def isValidUserEmail(self, new_data, all_data):
        ...
        try:
            ...
            raise validators.ValidationError, _(u"That e-mail address doesn't have an associated user account. Are you sure you've registered?")
    ...
    def save(self, domain_override=None, email_template_name='registration/password_reset_email.html'):
        ...
        send_mail(u'Password reset on %s' % site_name, t.render(Context(c)), None, [self.user_cache.email])
...
class PasswordChangeForm(oldforms.Manipulator):
    ...
    def __init__(self, user):
        ...
        self.fields = (
            ...
            oldforms.PasswordField(field_name="new_password1", length=30, max_length=30, is_required=True,
                validator_list=[validators.AlwaysMatchesOtherField('new_password2', _(u"The two 'new password' fields didn't match."))]),
    ...
    def isValidOldPassword(self, new_data, all_data):
        ...
        if not self.user.check_password(new_data):
            raise validators.ValidationError, _(u"Your old password was entered incorrectly. Please enter it again.")
