...
0.02  15.11.2007  Integration von min_role_id
0.03  16.11.2007  min_role_id nach dmsFolderProtected
0.04  08.04.2008  is_changeable
"""
...
def folder_edit(request, item_container):
  ...
  def save_values(item_container, old, new):
    ...
    if new.has_key('min_role_id') and item_container.container.min_role_id != int(new['min_role_id']):
  ...
  data_init = {
                ...
                'integer_1'      : my_item.integer_1,
                'min_role_id'    : item_container.container.min_role_id,
                'is_changeable'  : item_container.is_changeable,
              }
  if item_container.parent_item_id >= 0 :
  ...
  if user_role <= get_role_by_name('the_manager'):
    tab_more = ['show_next', 'string_1', 'integer_1', 'is_changeable', 'min_role_id' ]
  else:
