# -*- coding: utf-8 -*-
"""
/dms/newsletteritem/views_add.py

.. enthaelt den View zum Ergaenzen eines Beitrags zu einem Newsletter
...
from dms.newsletteritem.help_form  import help_form
...
# -----------------------------------------------------.
def newsletteritem_add(request, item_container):
  """ neuen Beitrag zu Newsletter anlegen """
  ...
  def save_values(name, new, my_folder):
    """ Daten sichern """
    #new['section'] = ''
    save_item_values(request.user, 'dmsNewsletterItem', name, new, my_folder,
                     not my_folder.item.is_moderated, True)
    #save_item_values(request.user, 'dmsNewsletterItem', name, new, my_folder,
    #                 not my_folder.item.is_moderated, False)
  ...
  my_title = _(u'Beitrag zum Newsletter anlegen')
  ...
  if request.method == 'POST' and not f.errors:
    ...
    if is_ok:
      ...
      name = 'newsletter_' + now[:string.find(now, '.')] + '.html'
