# -*- coding: utf-8 -*-
"""
/dms/eduscormitem/views_add.py

.. enthaelt den View zum Ergaenzen eines Scorm-Paketes in den Lernarchiven
...
0.01  11.03.2003  Beginn der Arbeit
"""
...
from dms.eduscormitem.utils     import save_scorm_packet

from dms_ext.extension  import * # dms-Funktionen ueberschreiben

# -----------------------------------------------------
@login_required
def eduscormitem_add(request, item_container):
  """ neue Datei fuer Lernarchiv anlegen """

  def save_values(name, new, files, item_container):
    """ Daten sichern """
    folder_name = name[:-4]
    save_scorm_packet(folder_name, name, files, item_container)
    # --- name ohne .zip
    name = folder_name
    community_id = 0
    schulverbund_id = 0
    ...
    item_container_new = save_item_values(request.user, 'dmsEduScormItem', name, new,
                                          item_container,
  ...
  my_title = _(u'Scorm-Paket im Lernarchiv anlegen')
  ...
  if request.method == 'POST' and not f.errors:
    ...
    # --- handelt es sich um eine Zip-Datei?
    if name.lower()[-4:] != '.zip':
      return show_error(request, item_container, _(u'Falsche Datei'), 
                        _(u'<p>Sie können nur Scorm-Dateien verwenden!</p>'))
    else:
