# -*- coding: utf-8 -*-
"""
/dms/eventitem/views_add.py

.. enthaelt den View zum Ergaenzen eines Termins
...
from dms.eventitem.help_form       import help_form

from dms_ext.extension    import * # dms-Funktionen ueberschreiben

# -----------------------------------------------------
def eventitem_add(request, item_container):
  ...
  def save_values(name, new, my_folder):
    ...
    save_item_values(request.user, 'dmsEventItem', name, new, my_folder,
  ...
  app_name = 'eventitem'
  my_title = _(u'Termin anlegen')
  # --- Sind Daten vorhanden oder muessen Sie initialisiert werden?
  show_errors = ( request.method == 'POST' )
  if show_errors:
    data = request.POST.copy()
  else :
    data = { 'visible_start': datetime.datetime.now().strftime('%d.%m.%Y'),
             'visible_end': datetime.datetime.now().strftime('%d.%m.%Y'), }
  f = DmsItemForm ( data )
  ...
  if request.method == 'POST' and not f.errors:
    ...
    if is_ok:
      ...
      name = 'event_item_' + now[:string.find(now, '.')] + '.html'
    ...
    if my_user == None:
      ...
      tabs = [ ('tab_base', [ 'string_1', 'string_2', 'title', 'sub_title', 'text',
                              'text_more', 'visible_start', 'visible_end', 'url_more',
