# -*- coding: utf-8 -*-
"""
/dms/edumediaitem/views_add.py

.. enthaelt den View zum Ergaenzen eines Medienpakets
...
0.02  24.09.2007  Eingabefelder wie bei Webquest
0.03  28.09.2007  if new.has_key ..
0.04  28.11.2007  eigenes Logo setzen
0.05  20.02.2008      new['integer_4'] = get_medienformat_by_name('Medienpaket').id
"""
...
def edumediaitem_add(request, item_container):
  ...
  def save_values(name, new, item_container):
    ...
    new['integer_4'] = get_medienformat_by_name('Medienpaket').id
    ...
    item_container_new = save_container_values(request.user, 'dmsEduMediaItem',
  ...
  my_title = _(u'Medienpaket anlegen')
  ...
  if request.method == 'POST':
    ...
    data = { 'schlagwort': schlagworte_str,
             'lernrestyp': 15, # Unterrichtsentwurf
             'sprache': sprache,
  ...
  if request.method=='POST' and not f.errors:
    ...
    if not exist_item(item_container, name):
      save_values(name, f.cleaned_data, item_container)
