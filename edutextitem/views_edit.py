# -*- coding: utf-8 -*-
"""
/dms/edutextitem/views_edit.py

.. enthaelt den View zum Aendern der Eigenschaften eines Textes innerhalb der Lernarchive
...
0.01  06.09.2007  Beginn der Arbeit
0.03  24.09.2007  is_exchangeable
0.04  05.05.2008  get_data_url
"""
...
from dms.edufolder.utils         import get_data_url
...
from dms.edutextitem.help_form   import help_form

from dms_ext.extension    import * # dms-Funktionen ueberschreiben

# -----------------------------------------------------
@require_permission('perm_edit')
def edutextitem_edit(request, item_container):
  """ Eigenschaften des Ordners aendern """

  @transaction.commit_manually
  def save_extra(item_container, old, new):
  ...
  def save_values(item_container, old, new):
    save_item(item_container, old, new, True, save_extra)
  ...
  class DmsItemForm ( forms.Form ) :
    ...
    if item_container.is_data_object:
      ...
      beschreibung_lang = forms.CharField(required=False,
            widget=forms.Textarea(attrs={'rows':20, 'cols':60, 'id':'ta1',
                                        'style':'width:100%;'}) )
  ...
  app_name = u'edutextitem'
  my_title = _(u'Textdokument innerhalb der Lernarchive ändern')
  ...
  if item_container.is_data_object:
    ...
    my_title = _(u'Einblendung eines Textdokuments im Lernarchiv ändern')
    tabs = [ ( 'tab_visibility', [ 'section', 'is_browseable', 'visible_start', 'visible_end', ] ), ]
    top_of_form = get_data_url(item_container)
    content = get_tabbed_form(tabs, help_form, app_name, f, top_of_form=top_of_form)
