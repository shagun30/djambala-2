# -*- coding: utf-8 -*-
"""
/dms/eduscormitem/views_show.py

.. zeigt den Inhalt eines Scorm-Paketes innerhalb der Online-Lernarchive an
...
from dms.eduscormitem.views_player  import scorm_index
from dms.eduscormitem.views_player  import scorm_left
from dms.eduscormitem.views_player  import scorm_top
...
def get_details(item_container):
  ...
  tDetails = get_template('app/eduscormitem/show_more.html')
  ...
  show_url = item_container.get_absolute_url() + '?show_index=1'
  file_url = '%s/%s.zip' % ( get_edu_file_url(item_container), item_container.item.name )
  details = Context( {
                       'eduobject_lernrestyp': get_lernrestyp_by_id(
                                                   item_container.item.integer_3).name,
                       'beschreibung_lang': data.beschreibung_lang,
                       'eduobject_show_url': show_link(show_url, show_url,
                                                       item_container.item.url_more_extern),
                       'eduobject_file_url': show_link(file_url),
...
# -----------------------------------------------------
def eduscormitem_show(request, item_container):
  """ zeigt Details des Verweises an """
  app_name = 'eduscormitem'
  if request.GET.has_key('show_index'):
    return scorm_index(request, item_container)
  if request.GET.has_key('show_left'):
    return scorm_left(request, item_container)
  if request.GET.has_key('show_top'):
    return scorm_top(request, item_container)
