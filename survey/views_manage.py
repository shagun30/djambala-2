...
def survey_manage(request, item_container):
  """ Pflegemodus des Fragebogens """

  user_perms = UserEditPerms(request.user.username, request.path)
  add_ons = {}
  add_ons[0] = [ { 'url' : get_site_url(item_container, 'index.html/add/surveyitem/?form_type=input'),
                   'info': _(u'Einzeiliges Antwortfeld')},
                 { 'url' : get_site_url(item_container, 'index.html/add/surveyitem/?form_type=text'),
                   'info': _(u'Mehrzeiliges Antwortfeld')},
                 { 'url' : get_site_url(item_container, 'index.html/add/surveyitem/?form_type=radio'),
                   'info': _(u'Einzelnes Auswahlfeld ("Radio-Button")')},
                 { 'url' : get_site_url(item_container, 'index.html/add/surveyitem/?form_type=checkbox'),
  ...
  app_name = 'survey'
  my_title = _(u'Fragebogen pflegen')
  my_title_own = ''
