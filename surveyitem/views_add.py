...
from dms.surveyitem.help_form   import help_form

from dms_ext.extension    import * # dms-Funktionen ueberschreiben

# -----------------------------------------------------
@require_permission('perm_add')
def surveyitem_add(request, item_container):
  ...
  def save_values(name, new, my_folder):
    ...
    if new.has_key('text_more'):
      new['text_more'] = new['text_more'].replace('<p>', '').replace('</p>', '')
    save_item_values(request.user, 'dmsSurveyItem', name, new, my_folder, True, False)
  ...
  class DmsItemForm(forms.Form):
    ...
    integer_2 = forms.IntegerField(required=False, min_value=1, max_value=200,
                                   widget=forms.TextInput(attrs={'size':5}) )
    integer_3 = forms.IntegerField(required=False, min_value=1, max_value=80,
                                   widget=forms.TextInput(attrs={'size':5}) )
    integer_4 = forms.IntegerField(required=False, min_value=20, max_value=60,
                                   widget=forms.TextInput(attrs={'size':5}) )
    integer_5 = forms.IntegerField(required=False, min_value=3, max_value=20,
                                   widget=forms.TextInput(attrs={'size':5}) )
