...
def freemind_edit(request, item_container):
  """ Eigenschaften der Freemind-Datei aendern """
  ...
  def save_values(item_container, old, new, files):
    ...
    if files != {}:
      ...
      if filename == item_container.item.name + '.mm':
        folder_name = item_container.item.name
        save_mindmap(folder_name, filename, files, item_container)
