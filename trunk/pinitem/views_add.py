# -*- coding: utf-8 -*-
"""
/dms/pinitem/views_add.py

.. enthaelt den View zum Ergaenzen eines Pinnwandbeitrags
...
# -----------------------------------------------------.
def pinitem_add(request, item_container):
  ...
  def save_values(name, new, my_folder):
    """ Daten sichern """
    save_item_values(request.user, 'dmsPinItem', name, new, my_folder,
  ...
  app_name = 'pinitem'
  my_title = _(u'Beitrag für Pinnwand anlegen')
  ...
  if request.method == 'POST' and not f.errors:
    ...
    if is_ok:
      ...
      name = 'pin_' + now[:string.find(now, '.')] + '.html'
      new = f.data
